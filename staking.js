window.Staking = (function () {
  ...
  async function updateMtecEstimate() {
    ...
    try {
      const router = await Wallet.getRouterContract();
      const amountIn = Utils.parseUnits(amountStr, thbcDecimals);
      const path = [CONFIG.thbc.address, CONFIG.mtec.address];

      const amounts = await router.getAmountsOut(amountIn, path);
      const out = Utils.formatUnits(amounts[1], mtecDecimals);
  ...
  async function swapThbcToMtec() {
    ...
    const allowance = await thbc.allowance(addr, CONFIG.router.address);

    if (allowance.lt(amountIn)) {
      const txApprove = await thbc.approve(CONFIG.router.address, amountIn);
      Utils.showToast("กำลัง Approve THBC...");
      await txApprove.wait();
    }

    // minOut with slippage
    const slippagePercent = Number(document.getElementById("selectSlippage").value || "2");
    const amountsOut = await router.getAmountsOut(amountIn, path);
    const expected = amountsOut[1];
    const minOut = expected.mul(ethers.BigNumber.from(100 - slippagePercent)).div(100);

    const deadline = Math.floor(Date.now() / 1000) + 60 * 20;

    Utils.showToast("กำลัง Swap THBC → MTEC...");
  ...
  async function loadStakingInfo() {
    ...
    document.getElementById("infoStakingReward").textContent =
      Utils.formatNumber(
        Utils.formatUnits(pending.stakingReward || pending[0] || 0, mtecDecimals),
        4
      ) + " MTEC";

    document.getElementById("infoReferralReward").textContent =
      Utils.formatNumber(
        Utils.formatUnits(pending.referralReward || pending[1] || 0, mtecDecimals),
        4
      ) + " MTEC";
